                ...
                if ( $challenge['type'] == 'dns-01' && stripos( $challenge['identifier'], $this->rootdomain ) !== FALSE ) {
                    ...
                    if ( $challenge['type'] == 'http-01' && stripos( $challenge['identifier'], $this->rootdomain ) !== FALSE ) {
                        ...
                        if ( $rsponse !== trim( $challenge['content'] ) ) {
                            ////$this->wple_log("Could not verify challenge code in above http challenge file. Trying again with rewrite rule!.\n", 'success', 'a');
                        ...
                        if ( $rsponse != trim( $challenge['content'] ) && !isset( $_GET['wpleauto'] ) ) {
                            // if (wple_fs()->can_use_premium_code__premium_only()) { //?acme was useless
                            //   if (is_writable(ABSPATH . '.htaccess')) {
                            //     $htaccess = file_get_contents(ABSPATH . '.htaccess');
                            //     $group = "/RewriteRule\s\^.well-known.*/";
                            ...
                            update_option( 'wple_error', 2 );
                            $this->wple_save_all_challenges();
                            ////$this->wple_verify_if_http_possible($challenge['identifier']);
